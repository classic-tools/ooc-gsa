...
your .emacs file, e.g. '(setq o2-cwd \"/users/home/my_home/oo2c/\")'")
...
;; the ending '.<extension>' is automatically appended. 
  (save-excursion
    ...
    (erase-buffer)
    (o2-execute-command (concat "%OOWHEREIS% " file-name))
    (goto-char (point-min))
...
;; Calls the external command oowhereis with the parameter modulename.Mod and reads
...
;; given symbol file. The buffer gets the name *modulename.Def* and is not selected.
  (save-excursion
	...
	(erase-buffer)
	(o2-execute-command (concat "%OOB% -x " module-name)))
